class window.LineItemsReportView
  constructor: (db, year) ->
    ...
    Lazy(rootCategoriesMap).values().uniq().each (categoryName) =>
  ...
  generateReport: =>
    ...
    Object.keys(@lineItemsBox.rowByHash).sort().forEach (categoryName) =>
      ...
      categoryInfo = {
        name: categoryName
        monthlyTotals: []
        avg: @lineItemsBox.rowTotals(categoryName).amount.times(-1).div(12).toFixed(2)
        total: @lineItemsBox.rowTotals(categoryName).amount.times(-1).toFixed(2),
        percent: @lineItemsBox.rowTotals(categoryName).amount.times(-1).div(totalExpenses).times(100).toFixed(0),
        categories: Object.keys(@rootCategoryToCategories[categoryName]).join(',')
      }
      @lineItemsBox.rowColumnValues(categoryName).forEach (expenseColumn) =>
